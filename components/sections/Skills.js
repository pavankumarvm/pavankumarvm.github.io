  ...
  return (
    <section className={`section ${styles.section}`} id="skills">
      <div className="container">
        ...
        <motion.div className={styles.grid} variants={stagger(0.08)} initial="hidden" whileInView="show" viewport={viewportOnce}>
          {Object.entries(skills).map(([category, items]) => {
            const color = categoryColors[category] || "#6c63ff";
            return (
              <motion.div key={category} className={`card ${styles.card}`} variants={scaleIn} whileHover={{ y: -4 }}>
                <div className={styles.catHeader}>
                  <span className={styles.catDot} style={{ background: color, boxShadow: `0 0 8px ${color}` }} />
                  <h3 className={styles.catTitle}>{category}</h3>
                </div>
                <div className={styles.items}>
                  {items.map((item) => (
                    <motion.span key={item} className={styles.skill} style={{ "--c": color }} whileHover={{ scale: 1.08, y: -2 }}>
